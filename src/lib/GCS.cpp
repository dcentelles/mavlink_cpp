...
void GCS::SendGPSOrigin(uint32_t lat, uint32_t lon) {
...
void GCS::SendGPSInput(mavlink_gps_input_t &msg) {
...
void GCS::SendSetPositionTargetLocalNED(
...
void GCS::EnableManualControl(bool enable) {
...
void GCS::SetManualMode() { SetFlyMode(FLY_MODE_R::MANUAL); }
...
void GCS::SetHome(double lat, double lon, double alt) {
  uint8_t txbuf[GCS_BUFFER_LENGTH];
...
void GCS::_RunNavModeWork() {
  std::thread work([this]() {
    while (true) {
      std::unique_lock<std::mutex> lock(_hb_mutex);
      while (!_hb_updated) {
        _hb_cond.wait(lock);
      }
      _CheckFlyMode(_desiredMode);
      _hb_updated = false;
    }
  });
  work.detach();
...
void GCS::SetFlyMode(FLY_MODE_R flymode) { _desiredMode = flymode; }

void GCS::_CheckFlyMode(FLY_MODE_R flymode) {
...
void GCS::SendVisionPositionEstimate(mavlink_vision_position_estimate_t &msg) {
  uint8_t txbuf[GCS_BUFFER_LENGTH];
  memset(txbuf, 0, GCS_BUFFER_LENGTH);
  mavlink_message_t auxMsg;
...
void GCS::SendGPSFix(mavlink_hil_gps_t &msg) {
  uint8_t txbuf[GCS_BUFFER_LENGTH];
  memset(txbuf, 0, GCS_BUFFER_LENGTH);
  mavlink_message_t auxMsg;
...
void GCS::DebugGlobalPositionInt(mavlink_global_position_int_t &msg) {
...
void GCS::DebugLocalPositonNED(mavlink_local_position_ned_t &msg) {
...
void GCS::WaitForNEDUpdate() {
  std::unique_lock<std::mutex> lock(_lposned_mutex);
  while (!_lposned_updated && !_gposint_validOrigin) {
    _lposned_cond.wait(lock);
  }
}
mavlink_global_position_int_t GCS::GetGPS() { return _gposint; }
mavlink_local_position_ned_t GCS::GetNED() { return _lposned; }
mavlink_scaled_imu2_t GCS::GetScaledIMU2() { return _scaledImu2; }

mavlink_attitude_quaternion_t GCS::GetAttitudeQuaternion() {
  return _attitudeQuaternion;
}
mavlink_attitude_t GCS::GetAttitude() { return _attitude; }
mavlink_vfr_hud_t GCS::GetVfrHud() {
  _vfr_hud_mutex.lock();
...
void GCS::DebugScaledIMU2(mavlink_scaled_imu2_t &msg) {
  Debug("SCALED_IMU:"
        "\ttime_boot_ms: {}\n"
        "\txacc: {}\n"
        "\tyacc: {}\n"
...
void GCS::DebugRawIMU(mavlink_raw_imu_t &msg) {
  Debug("RAW_IMU:"
        "\ttime_usec: {}\n"
        "\txacc: {}\n"
        "\tyacc: {}\n"
...
void GCS::DebugAttitudeQuaternion(mavlink_attitude_quaternion_t &msg) {
...
void GCS::DebugAttitude(mavlink_attitude_t &msg) {
  Debug("ATTITUDE:"
        "\ttime_boot_ms: {}\n"
        "\troll: {}\n"
        "\tpitch: {}\n"
